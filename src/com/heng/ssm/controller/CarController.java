...
import com.heng.ssm.entity.Car;
import com.heng.ssm.entity.Item;
import com.heng.ssm.service.CarService;
...
public class CarController {
    ...
    public String exAdd(Car car, HttpServletRequest request) {
        ...
        car.setPrice(itemPrice);
        if (item.getZk() != null) {
            itemPrice = itemPrice * item.getZk() / 10;
            BigDecimal bg = new BigDecimal(itemPrice).setScale(2, RoundingMode.UP);
            car.setPrice(bg.doubleValue());
            itemPrice = bg.doubleValue();
        }
        Integer num = car.getNum();
        Double carPrice = itemPrice * num;

        BigDecimal bg = new BigDecimal(carPrice).setScale(2, RoundingMode.UP);
